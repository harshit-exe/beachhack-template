...
export default function IncomingCallModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden animate-bounce-soft">
        {/* Header with ringing animation */}
        <div className={`px-6 py-8 text-center ${isVIP ? 'bg-gradient-to-r from-amber-500 to-orange-500' : 'bg-gradient-to-r from-green-500 to-emerald-500'}`}>
          <div className="relative inline-block">
            {/* Pulse rings */}
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="absolute inline-flex h-20 w-20 rounded-full bg-white/30 animate-ping"></span>
              <span className="absolute inline-flex h-16 w-16 rounded-full bg-white/40 animate-ping" style={{ animationDelay: '0.2s' }}></span>
            </div>
        ...
        <div className="px-6 py-6">
          <div className="flex items-center gap-4 mb-4">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white text-xl font-bold ${
              isVIP ? 'bg-gradient-to-br from-amber-500 to-orange-600' : 'bg-gradient-to-br from-blue-500 to-purple-600'
            }`}>
              {customer.name 
            ...
            <div className="space-y-2 mb-4 p-4 bg-gray-50 rounded-xl">
              ...
              {customer.metadata?.notes && (
                <div className="mt-2 p-3 bg-purple-50 border border-purple-200 rounded-lg">
                  <p className="text-xs font-semibold text-purple-700 mb-1">📋 Previous Call Context:</p>
                  <p className="text-sm text-purple-800">{customer.metadata.notes}</p>
                </div>
              )}
              ...
              {customer.metadata?.scheduledMeeting && (
                <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-xs font-semibold text-green-700 mb-1">📅 Scheduled Meeting:</p>
                  <p className="text-sm text-green-800">{customer.metadata.scheduledMeeting}</p>
                </div>
              )}
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={onAnswer}
              className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 transition-colors shadow-lg"
            >
            ...
            <div className="flex gap-3">
              ...
              {onSendToAI && (
                <button
                  onClick={onSendToAI}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-purple-100 text-purple-700 rounded-xl font-semibold hover:bg-purple-200 transition-colors"
                >
